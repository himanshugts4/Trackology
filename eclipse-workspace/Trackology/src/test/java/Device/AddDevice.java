...
public class AddDevice {
	...
	public void loginAndAddDeviceTest() throws InterruptedException {
		...
		// Add New Device
		WebElement addDeviceBtn = wait.until(
				ExpectedConditions.elementToBeClickable(By.xpath("//button[normalize-space()='Add New Device']")));
		addDeviceBtn.click();
		System.out.println("✅ Clicked on Add New Device");

		// Add Manually
		WebElement addManually = wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath("//li[normalize-space()='Add Manually']")));
		addManually.click();
		System.out.println("✅ Clicked on Add Manually");

		// Enter IMEI
		WebElement imeiField = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@name='imei']")));
		...
		WebElement durationDropdown = wait.until(ExpectedConditions.elementToBeClickable(By.xpath(
				"//div[@role='combobox' and contains(@class, 'MuiSelect-select') and not(contains(@class, 'Mui-disabled'))]")));
		...
		try {
			...
			wait.until(
					ExpectedConditions.invisibilityOfElementLocated(By.xpath("//ul[contains(@class,'MuiMenu-list')]")));
		...
		WebElement genderOption = wait.until(ExpectedConditions
				.visibilityOfElementLocated(By.xpath("//li[@role='option' and normalize-space()='Male']")));
		 ...
		 * pin2Image.click(); System.out.println("✅ Pin2 image clicked successfully.");
		 * 
		 */

		WebDriverWait wait13 = new WebDriverWait(driver, Duration.ofSeconds(15));
		WebElement addDeviceBtn1 = wait
				.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[normalize-space()='Add Device']")));
		addDeviceBtn1.click();
